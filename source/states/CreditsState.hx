...
class CreditsState extends SuffState {
	...
	override public function create():Void {
		...
		for (index => line in creditsTxt) {
			...
			if (lineType != 'LOGO') {
				...
				leChar.setFormat(Paths.font('default', false), leSize, leColor);
			}
	...
	function loadCreditsArt(artId:String = 'nicklysuffer') {
		...
		var graphic = Paths.image('ui/menus/credits/art/$artId');
		if (graphic == null) graphic = Paths.image('ui/menus/credits/art/empty');
